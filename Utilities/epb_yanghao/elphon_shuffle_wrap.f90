  ...
  USE pwcom,         ONLY : nks, nbnd, nkstot, nelec, xk
  ...
  USE phcom,         ONLY : evq, nq1, nq2, nq3
...
#endif
  !! YHC: write epmat in HDF5
  USE qeh5_base_module,  ONLY  : qeh5_file, qeh5_dataset, qeh5_openfile,&
                                 qeh5_open_dataset, qeh5_add_attribute,&
                                 qeh5_write_dataset, qeh5_close, qeh5_set_space,&
  ...
  !! YHC: write HDF5
  CHARACTER(len=256)  :: temphdf5
  TYPE (qeh5_file)    :: h5file
  TYPE (qeh5_dataset) :: epmat_dset, dynq_dset, xqc_dset,&
                         et_dset, xk_dset, epsi_dset, zstar_dset,&
                         ityp_dset, amass_dset, bg_dset
...
15  FORMAT(5x,'Estimated size of gmap: ngxx =', i5)
  ...
  IF (my_image_id == 0) THEN
    IF (epbread .OR. epbwrite) THEN
      ...
      !! YHC
      CALL set_ndnmbr(0, my_pool_id+1, 1, npool, filelab)
      temphdf5 = trim(tmp_dir) // trim(prefix) // '_epb' // trim(filelab) // '.h5'
      IF (epbwrite) THEN
         CALL qeh5_openfile(h5file, temphdf5, action = 'write') 
         CALL qeh5_add_attribute( h5file%id, "nq", nqc )
         CALL qeh5_add_attribute( h5file%id, "nbnd", nbnd )
         CALL qeh5_add_attribute( h5file%id, "nbndep", nbndep )
         CALL qeh5_add_attribute( h5file%id, "my_nk", nks )
         CALL qeh5_add_attribute( h5file%id, "nk", nkstot )
         CALL qeh5_add_attribute( h5file%id, "nmodes", nmodes )
         CALL qeh5_add_attribute( h5file%id, "nat", nat )

         CALL qeh5_set_space( amass_dset, amass(1), 1, [nat], MODE = 'f')
         CALL qeh5_open_dataset (h5file, amass_dset, ACTION = 'write', NAME = 'amass' )
         CALL qeh5_write_dataset ( amass, amass_dset)  
         CALL qeh5_close ( amass_dset) 

         CALL qeh5_set_space( ityp_dset, ityp(1), 1, [nat], MODE = 'f')
         CALL qeh5_open_dataset (h5file, ityp_dset, ACTION = 'write', NAME = 'ityp' )
         CALL qeh5_write_dataset ( ityp, ityp_dset)   
         CALL qeh5_close ( ityp_dset) 

         CALL qeh5_set_space( bg_dset, bg(1,1), 2, [3, 3], MODE = 'f')
         CALL qeh5_open_dataset (h5file, bg_dset, ACTION = 'write', NAME = 'bvec' )
         CALL qeh5_write_dataset ( bg, bg_dset)   
         CALL qeh5_close ( bg_dset) 

         CALL qeh5_set_space( xqc_dset, xqc(1,1), 2, [3, nq1*nq2*nq3], MODE = 'f')
         CALL qeh5_open_dataset (h5file, xqc_dset, ACTION = 'write', NAME = 'xqc' )
         CALL qeh5_write_dataset ( xqc, xqc_dset)   
         CALL qeh5_close ( xqc_dset) 

         CALL qeh5_set_space( et_dset, et_all(1,1), 2, [nbnd, nkstot], MODE = 'f')
         CALL qeh5_open_dataset (h5file, et_dset, ACTION = 'write', NAME = 'et' )
         CALL qeh5_write_dataset ( et_all, et_dset)   
         CALL qeh5_close ( et_dset) 

         CALL qeh5_set_space( xk_dset, xk(1,1), 2, [3, nks], MODE = 'f')
         CALL qeh5_open_dataset (h5file, xk_dset, ACTION = 'write', NAME = 'my_xk' )
         CALL qeh5_write_dataset ( xk, xk_dset)   
         CALL qeh5_close ( xk_dset) 

         CALL qeh5_set_space( epsi_dset, epsi(1,1), 2, [3, 3], MODE = 'f')
         CALL qeh5_open_dataset (h5file, epsi_dset, ACTION = 'write', NAME = 'epsi' )
         CALL qeh5_write_dataset ( epsi, epsi_dset)   
         CALL qeh5_close ( epsi_dset) 

         CALL qeh5_set_space( zstar_dset, zstar(1,1,1), 3, [3, 3,nat], MODE = 'f')
         CALL qeh5_open_dataset (h5file, zstar_dset, ACTION = 'write', NAME = 'zstar' )
         CALL qeh5_write_dataset ( zstar, zstar_dset)   
         CALL qeh5_close ( zstar_dset) 

         CALL qeh5_set_space( dynq_dset, dynq(1,1,1), 3, [nmodes,nmodes,nq1*nq2*nq3], MODE = 'f')
         CALL qeh5_open_dataset (h5file, dynq_dset, ACTION = 'write', NAME = 'dynq' )
         CALL qeh5_write_dataset ( dynq, dynq_dset)   
         CALL qeh5_close ( dynq_dset) 

         CALL qeh5_set_space( epmat_dset, epmatq(1,1,1,1,1), 5,&
                              [nbndep, nbndep, nks, nmodes, nqc1*nqc2*nqc3], MODE = 'f')
         CALL qeh5_open_dataset (h5file, epmat_dset, ACTION = 'write', NAME = 'epmatq' )
         CALL qeh5_write_dataset ( epmatq, epmat_dset)   
         CALL qeh5_close ( epmat_dset) 
  ...
  !    DO ik = 1,nks
  !    write(*,*) 'ik', ik
  !    write(*,*)'epmatq(:,:,ik,:,iq)',SUM(epmatq(:,:,ik,:,iq))
  !    write(*,*)'epmatq(:,:,ik,:,iq)**2',SUM((REAL(REAL(epmatq(:,:,ik,:,iq))))**2)+&
  !               SUM((REAL(AIMAG(epmatq(:,:,ik,:,iq))))**2)
  !    ENDDO
  !  ENDDO
  !ENDIF
  !END
