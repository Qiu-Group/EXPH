...
g_qp = np.array([[0,0],[0,0]]) # .shape = (2,2)
...
V_x = np.array([0.3, 0.3])[:,np.newaxis,np.newaxis] # .shape(2,1,1)
V_y = np.array([0.2, 0.2])[:,np.newaxis,np.newaxis] # .shape(2,1,1)
...
def animate(i):
    ...
    #                                     80))
    plt.contourf(ini_xx, ini_yy, F_qxy_res[0, :, :, i], levels=np.linspace( 0.0, 0.16,80))

    plt.title('t=%s fs; ' % int(i * delta_T) + 'exciton number: %.2f'%F_qxy_res[ 0, :, :, i].sum())
    plt.colorbar()

    plt.subplot(1,2,2)
    plt.contourf(ini_xx, ini_yy, F_qxy_res[1, :, :, i], levels=np.linspace( 0.0, 0.16,80))
