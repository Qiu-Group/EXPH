 ...
 - Required package:
    ...
    - Install [EPW](https://docs.epw-code.org/doc/DownloadAndInstall.html) (I recommend you use EPW-5.3.0 and qe-6.6, which are 
     ...
     set hdf5 in configure that directs to HDF5 in Frontera: ./configure --with-hdf5=${TACC_HDF5_DIR}
      ...
      - ``cp ./EPW_src_modified/elphon_shuffle_wrap.f90 ~/software/your_qe_path/EPW/src/`` ``cp ./EPW_src_modified/EPW_summary.f90 ~/software/your_qe_path/EPW/src/``(This modified Fortran file could let EPW generate electron-phonon matrix with phase). 
  ...
  */3-exciton-band/1-mf/kpt.dat*. Note: this step is really **expensive**, please make sure everything is right before you 
  ...
  - `cd 3-exciton-band/2-bgw/` and you will find 2 directories:  
    ...
    **IMPORTANT**: please recalculate this with a very small cutoff energy (e.g. 7 Ry), even if you have already generated
    ...
    - ***2-sigma***: this directory includes *eqp.dat*, which will be used in later for interpolation. Luckily, you don't have 
    to recalculate it, just use your standard GW result (make sure k-grid here is the same as *../1-mf/2.1-wfn*).
  
  - execute this in command line to generate necessary directories and input files :  
  `finite_any_Q_BSE.py 2 2`  
  where the first "2" is number of conduction bands
  ...
  and you will find a bunch of directories including *inteqp* and *Q-n*.
    ...
    - ***Q-n***: BSE for each finite Q-exciton
      - ***kpt***: negative Q-shift for this BSE
...
By now, we have finished all calculation for electron-phonon matrix! The next step is to do exciton-phonon calculation.   
  ``cd ../5-exph/`` 
